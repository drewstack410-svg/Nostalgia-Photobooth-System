...
const hasPreviewFilter = computed(
  () => !!previewMatrix.value || !!cubeCurves.value,
);
...
// Film-grain overlay shows in the live preview (and gets baked into the
// captured photo — see applyGrain() in capturePhoto) whenever the
// selected filter's admin-configured grain toggle is on (Settings →
// Filters), so the preview always matches what capture bakes in.
const hasGrainOverlay = computed(() =>
  selectedFilter.value ? store.filterWantsGrain(selectedFilter.value) : false,
);
...
watch(effectiveFrameStyle, async (style) => {
  if (style === "blur" && stream.value) {
    await nextTick();
...
async function initCamera() {
  try {
    ...
    // no Canon connection, no live view, placeholder captures. Lets the
    // full flow be walked without the physical camera.
    if (!store.cameraDetectionEnabled) {
      console.log('[Camera] Camera detection OFF — running in test mode (placeholder captures)');
      cameraReady.value = true;
      return;
    }
...
function runCountdownAndCapture() {
  ...
  captureInterval = setInterval(() => {
    // Inactivity is suspended for the whole sequence via sequenceActive,
    // so there is nothing to reset per tick.
    ...
    if (countdownValue.value <= 0) {
      ...
      // isCapturing (not isCountingDown) covers the actual shutter+
      // processing time — without a separate flag here, the Start
      // button re-enables the instant the numeric countdown ends,
      // letting a fast double-tap fire a second concurrent capture
      // while the first one is still in flight.
      isCapturing.value = true;
      capturePhoto().then((success) => {
        isCapturing.value = false;
        // Only chain into the next shot on a successful capture and
        // while photos are still needed — a failed capture already
        // shows the retry modal (see capturePhoto's catch below), and
        // hasAllPhotos being true means capturePhotoInner is already
        // navigating to /printing.
        if (success && !store.hasAllPhotos) {
          runCountdownAndCapture();
        } else {
          // Sequence over (done, failed, or navigating to /printing) —
          // let the inactivity clock run again.
          endSequence();
        }
      });
    }
...
async function capturePhoto(): Promise<boolean> {
  // Pause live view during capture, restart after
  const hadLiveView = liveViewActive.value;
  console.log(
    `[Camera] capture ${store.capturedPhotos.length + 1}/${store.requiredPhotos} — ` +
      `hadLiveView=${hadLiveView}, canonConnected=${canonCameraConnected.value}`,
  );
  if (hadLiveView) await stopLiveView();

  try {
    await capturePhotoInner(hadLiveView);
    return true;
  } catch (error) {
    ...
    if (hadLiveView) await startLiveView();
    cameraErrorMessage.value =
...
// Draws a labelled placeholder "photo" (test mode only) at the camera's
// native 3:2 aspect so it flows through the print/QR pipeline exactly
// like a real capture.
function makeTestCapture(n: number): string {
  const w = 1800;
  const h = 1200;
  ...
  const palette = ["#8b7355", "#6b8e6b", "#7a6a9e", "#b5836a", "#5f8a9e", "#9e7a5f"];
  ctx.fillStyle = palette[(n - 1) % palette.length];
  ...
  ctx.fillText("TEST CAPTURE", w / 2, h / 2 + 120);
...
async function capturePhotoInner(hadLiveView: boolean) {
  ...
  // Test mode (camera detection off): synthesise a placeholder capture
  // so the rest of the flow (print, QR, reprint) can be exercised
  // without the physical camera.
  if (!store.cameraDetectionEnabled) {
    const dataUrl = makeTestCapture(store.capturedPhotos.length + 1);
    store.addPhoto(dataUrl);
    if (store.hasAllPhotos) {
      setTimeout(() => router.push("/printing"), 1000);
    }
    return;
  }

  // Use Canon EDSDK to capture photo
  if (canonCameraConnected.value && window.electronAPI?.canonTakePhoto) {
    try {
      ...
      if (result.success && result.imageData) {
        console.log(`[Camera] ✓ Photo captured from Canon camera: ${result.fileName}, imageData length=${result.imageData.length}, prefix="${result.imageData.slice(0, 40)}"`);
        ...
        if (canvas.width === 0 || canvas.height === 0) {
          throw new Error(`Canvas is ${canvas.width}x${canvas.height} — image did not load correctly (img was ${img.width}x${img.height})`);
        }
        ...
        // Bake in film grain when this filter's admin-configured grain
        // toggle is on (Settings → Filters) — mirrors hasGrainOverlay's
        // condition below, so the live preview overlay and the baked
        // capture always agree.
        const wantsGrain = filter ? store.filterWantsGrain(filter) : false;
        if (wantsGrain) {
          console.log("[Camera] Applying film grain");
          applyGrain();
        }

        const finalImageData = canvas.toDataURL("image/jpeg", 0.92);
        console.log(`[Camera] Canon image processed: ${Math.round(finalImageData.length / 1024)}KB (${canvas.width}x${canvas.height})`);
        store.addPhoto(finalImageData);

        // Restart live view after capture (unless all photos done)
        ...
        return; // Successfully captured with Canon camera
      } else {
...
// Set in onUnmounted so no deferred callback touches a dead component.
  ...
  <div class="camera-screen">
    ...
    <div class="camera-container">
      ...
      >
        ...
        ><img v-if="liveViewFrame" :src="liveViewFrame" class="liveview-img" :class="{ mirror: store.mirrorMode }" /><div v-if="overlayStyle" class="filter-overlay" :style="overlayStyle" aria-hidden="true"></div><div v-if="hasGrainOverlay" class="film-grain-overlay" aria-hidden="true"></div></div>
        ...
        <template v-if="effectiveFrameStyle === 'blur'">
          <div class="camera-feed-sharp-wrap">
            ...
            >
              <img v-if="liveViewFrame" :src="liveViewFrame" class="liveview-img" :class="{ mirror: store.mirrorMode }" />
              ...
              <div v-if="overlayStyle" class="filter-overlay" :style="overlayStyle" aria-hidden="true"></div><div v-if="hasGrainOverlay" class="film-grain-overlay" aria-hidden="true"></div>
            </div>
          </div>
        </template>
        ...
        <div v-else class="camera-feed camera-feed-liveview" :style="{ filter: livePreviewFilter }">
          ...
          <div v-else class="liveview-placeholder">
            <div class="liveview-placeholder-text">{{ !store.cameraDetectionEnabled ? 'Test mode — camera off. Press Start to use placeholder photos.' : cameraReady ? 'Starting preview...' : 'Connecting camera...' }}</div>
          </div>
