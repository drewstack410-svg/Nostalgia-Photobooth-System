...
if (edsdkAvailable) {
  ...
  ipcMain.handle('canon-take-photo', async () => {
    try {
      ...
      // Always tear live view down and always wait. The old conditional was
      // effectively dead code: CameraView.capturePhoto() already invokes
      // canon-stop-liveview before this handler runs, so the flag read false
      // here and the 700ms settle was skipped on every real capture. (The
      // flag is also a CACHED value that only flips on an async EDSDK
      // Evf_OutputDevice property event, so it lags the real state.)
